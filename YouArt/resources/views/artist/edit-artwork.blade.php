...
<div class="container mx-auto px-4 py-8 bg-cream">
    <div class="max-w-2xl mx-auto bg-sand rounded-lg shadow-md p-8">
        <h1 class="text-3xl font-bold mb-8 text-rust serif flex items-center">
            <i class="fas fa-edit mr-3"></i>Edit Artwork
        </h1>
        ...
        <form action="{{ route('artworks.update', $artwork->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <div class="flex items-center space-x-4 mb-3">
                    <div class="border-4 border-cream rounded-md overflow-hidden shadow-md">
                        <img src="{{ asset('storage/' . $artwork->image_path) }}" alt="{{ $artwork->title }}" class="w-32 h-32 object-cover">
                    </div>
                    <div class="text-coffee text-sm italic">
                        <p>Current image</p>
                    </div>
                </div>
                ...
                <p class="mt-1 text-xs text-coffee italic flex items-center">
                    <i class="fas fa-info-circle mr-1"></i> JPG, PNG or GIF. Max 5MB. Leave empty to keep the current image.
                </p>
            </div>
            ...
            <div class="mb-5">
                <label for="is_sold" class="block mb-2 text-coffee font-medium flex items-center">
            ...
            <div class="flex justify-between">
                <div>
                    <button type="button" class="px-5 py-3 bg-rust bg-opacity-10 text-rust border border-rust border-opacity-30 rounded-md hover:bg-rust hover:text-cream transition flex items-center" 
                            onclick="document.getElementById('delete-form').submit();">
                        <i class="fas fa-trash-alt mr-2"></i>Delete Artwork
                    </button>
                </div>
